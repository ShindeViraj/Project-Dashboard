                    ...
                    <?php if(!empty($team)): ?>
                        <?php foreach($team as $member): ?>
                            <li class="list-group-item bg-transparent text-white border-light d-flex justify-content-between align-items-center">
                                <div>
                                    <i class="fas fa-user-circle me-2"></i> <?= htmlspecialchars($member['name']) ?>
                                    <span class="badge glass-card ms-1" style="font-size: 0.6rem;"><?= $member['is_leader'] ? 'Leader' : 'Member' ?></span>
                                </div>
                                <div class="d-flex align-items-center">
                                    <?php if($isLeaderOrAdmin && !$member['is_leader']): ?>
                                        <form action="/project/<?= htmlspecialchars($project['id']) ?>/team/<?= $member['id'] ?>/toggle-access" method="POST" class="me-3">
                ...
                <ul class="list-group list-group-flush mb-3 rounded overflow-hidden">
                    <?php if(!empty($team)): ?>
                        <?php foreach($team as $member): ?>
                            <li class="list-group-item bg-transparent text-white border-light d-flex justify-content-between align-items-center">
                                <div>
                                    <i class="fas fa-user-circle me-2"></i> <?= htmlspecialchars($member['user_name']) ?>
                                    <span class="badge glass-card ms-1" style="font-size: 0.6rem;"><?= htmlspecialchars($member['role']) ?></span>
                                </div>
                                <?php if($isLeader && $member['role'] !== 'leader'): ?>
                                    <form action="/project/<?= htmlspecialchars($project['id']) ?>/team/remove" method="POST" class="d-inline confirm-submit" data-confirm-msg="Remove member?">
                                        <input type="hidden" name="csrf_token" value="<?= htmlspecialchars($_SESSION['csrf_token'] ?? '') ?>">
                        ...
                        <?php foreach($moms as $mom): ?>
                            <div class="glass-card-dark p-2 mb-2 rounded">
                                <div class="d-flex justify-content-between">
                                    ...
                                    <small class="text-light"><?= htmlspecialchars($mom['author_name']) ?></small>
                                </div>
                    ...
                    <div class="glass-card mb-3 p-3 <?= ($step['is_applicable'] && $step['is_completed']) ? 'border-success' : '' ?>" style="border-width: 2px;">
                        ...
                        <div class="collapse mt-3" id="subtasks_<?= $step['id'] ?>">
                            <div class="glass-card-dark p-3 rounded">
                                <?php 
                                    // Normally we would have subtasks array grouped by step_id in the view
...
SESSION['csrf_token'] ?? '') ?>">
                                            <div class="form-check form-switch m-0">
                                                <input class="form-check-input" type="checkbox" onchange="this.form.submit()" <?= $member['can_edit'] ? 'checked' : '' ?>>
...
SESSION['csrf_token'] ?? '') ?>">
                                    ...
                                    <?php elseif (!$member['is_leader']): ?>
                                        <span class="text-light" style="font-size: 0.8rem;"><?= $member['can_edit'] ? 'Can Edit' : 'Read Only' ?></span>
                                    <?php endif; ?>
                                </div>
                            </li>
                        <?php endforeach; ?>
